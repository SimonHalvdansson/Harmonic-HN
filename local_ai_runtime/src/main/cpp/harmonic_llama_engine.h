...
void harmonic_llama_backend_initialize(
        harmonic_llama_log_callback callback,
        void * user_data);

/** Creates an independent inference session. The caller owns the returned handle. */
harmonic_llama_engine * harmonic_llama_create(void);

void harmonic_llama_destroy(harmonic_llama_engine * engine);

int harmonic_llama_load(
        harmonic_llama_engine * engine,
        const char * model_path,
        int context_tokens);

int harmonic_llama_start(
        harmonic_llama_engine * engine,
 ...
 */
harmonic_llama_next_result harmonic_llama_next(
        harmonic_llama_engine * engine,
        const char ** utf8_piece,
        size_t * utf8_length);

const char * harmonic_llama_last_error(const harmonic_llama_engine * engine);

void harmonic_llama_close(harmonic_llama_engine * engine);
