...
object ArxivResolver {
     ...
     */
    fun resolveSubject(abbreviation: String?): String? {
        return ARXIV_SUBJECTS.get(abbreviation)
    }

    fun isArxivSubject(abbr: String?): Boolean {
        return ARXIV_SUBJECTS.containsKey(abbr)
    }

    fun resolveFull(category: String?): String {
        return resolveSubject(category) + " (" + category + ")"
    }
}
