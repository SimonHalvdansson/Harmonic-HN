 ...
 */
class StoriesFeatureStore internal constructor(
...
) : FeatureStore<StoriesIntent, StoriesState, StoriesFeatureEffect> {
    private val mutableState = MutableStateFlow(snapshot())
    override val state: StateFlow<StoriesState> = mutableState.asStateFlow()
    private val mutableEffects = MutableSharedFlow<StoriesFeatureEffect>(extraBufferCapacity = 64)
    override val effects: SharedFlow<StoriesFeatureEffect> = mutableEffects.asSharedFlow()
    ...
    override fun accept(intent: StoriesIntent) {
        ...
        when (intent) {
            ...
            is StoriesIntent.More -> runtime.handleMenuAction(intent.action)
