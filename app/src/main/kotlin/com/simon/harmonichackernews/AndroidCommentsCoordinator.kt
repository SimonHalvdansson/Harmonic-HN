...
private class CommentsViewSession(
    ...
    val webViewController: AndroidCommentsWebViewController,
) {
    var backPressedCallback: OnBackPressedCallback? = null
    var composeController: CommentsScreenController? = null
}

class AndroidCommentsCoordinator(
    ...
    private val commentsStore = appComposition.createCommentsFeatureStore(
        CommentsFeatureHost(
    ...
    private val webViewController: AndroidCommentsWebViewController?
    ...
    private val composeController: CommentsScreenController?
    ...
    internal val composeUiController: CommentsScreenController?
    ...
    private fun initializeView(host: CommentsWebViewHost, savedInstanceState: Bundle?) {
        ...
        val webViewController = AndroidCommentsWebViewController(
            ...
            coroutineScope,
            object : AndroidCommentsWebViewController.Callbacks {
                ...
                override fun showMessage(
                    ...
                    this@AndroidCommentsCoordinator.showMessage(message, duration)
                }
                ...
                override fun syncOnBackPressedCallbackEnabledState() {
                    this@AndroidCommentsCoordinator.syncOnBackPressedCallbackEnabledState()
                }
    ...
    private fun createBackPressedCallback(
        webViewController: AndroidCommentsWebViewController,
    ...
    private fun installWindowInsetsListener(view: View) {
        ...
        AndroidViewInsets.requestApplyInsetsWhenAttached(view)
    }
    ...
    private fun initializeComposeUi() {
        ...
        navigation.attachCommentsScreenController(this, controller)
    ...
    private fun startComposeSummary() {
        ...
        webViewController?.getLoadedPageText(
            AndroidCommentsWebViewController.PageTextCallback(beginSummary),
        ) ?: beginSummary(null)
    }
        ...
        if (state.themeRefreshVersion != appliedCommentsThemeVersion) {
            ...
            activeContext?.let { currentContext ->
                webViewController?.setContainerBackgroundColor(
                    AndroidActivityTheme.getPageBackgroundColor(currentContext),
                )
            }
        }
        syncOnBackPressedCallbackEnabledState()
    ...
    fun onDestroy() {
        ...
        if (controllerToDetach != null) {
            navigation.detachCommentsScreenController(this, controllerToDetach)
        }
            ...
            rootView.post(Runnable {
                composeController?.let { controller ->
                    val backgroundColor = if (statusBars.headerColor != Color.TRANSPARENT)
                        ...
                        AndroidActivityTheme.getPageBackgroundColor(activity)
    ...
    private fun handleCommentsRuntimeEffect(effect: CommentsFeatureEffect) {
        when (effect) {
            is CommentsFeatureEffect.Platform -> handleCommentsPlatformEffect(effect.effect)
            is CommentsFeatureEffect.StateChanged -> syncComposeState()
            is CommentsFeatureEffect.ShowCommentActions ->
    ...
    private fun retryComposeSummaryWithWebView() {
        ...
        controller.requestSummary(
            AndroidCommentsWebViewController.PageTextCallback(commentsStore::startSummary),
        )
    }
