...
class AndroidStoriesCoordinator(
    ...
    var composeController: StoriesScreenController? = null
        private set
    private val storiesStore = appComposition.createStoriesFeatureStore(
        StoriesFeatureHost(
    ...
    private fun initializeComposeUi() {
        ...
        composeController = StoriesScreenController.create(
        ...
        navigation.attachStoriesScreenController(composeController!!)
        syncComposeState()
    }
    ...
    private fun handleStoriesRuntimeEffect(effect: StoriesFeatureEffect) {
        when (effect) {
            is StoriesFeatureEffect.OpenStory -> navigation.openStory(effect.destination)
            is StoriesFeatureEffect.OpenExternalLink ->
                navigation.scene.links.openExternal(ExternalLinkRequest(effect.url))
            is StoriesFeatureEffect.Platform -> handleStoriesPlatformEffect(effect.effect)
    ...
    private fun syncComposeState() {
        ...
        val state = storiesStore.state.value
        val presentation = storiesPlatformPresentation(state)
    ...
    private fun clearControllerReferences() {
        ...
        if (controller != null) {
            navigation.detachStoriesScreenController(controller)
        }
